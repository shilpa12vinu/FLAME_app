...
int  main( ) 
{
	...
	for(j=0;j<=tot_step;j=j+1)                               /*For each time step*/
          {
            u[0]=u[n]=0;   	                                 /*BC:Assigning 0 to the end points*/
            for(k=0;k<=n;k++)
		{
                    if(k==0)
                       {
		...
		for(k=0;k<=n;k++)
		       {
		         u[k]=u[k]+((dudt[k])*dt);                  /*Forward in time*/ 
                       }
    
            /*Section 3: Computing Analytical value of diffusion, average and  absolute error */

             t1=time[temp];
             if(j==t1)
               {   
                 char new_file[]="trial";                          /*Writting a file with required  parameters*/
                 sprintf(file_index,"%d",time[temp]);              /*Assigning the file name as per the time step*/
                 ...
                 for(k=0;k<=n;k++)                                 /*Through each nodal points*/
                    {
                      ...
                      err[k]=(u_a[k]-u[k]);                        /*Computing the  absolute error*/
